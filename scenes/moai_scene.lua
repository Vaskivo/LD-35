...
function Scene.setup(self, ...)
  ...
  deck:setTexture(ResourceManager.getSprite('moaiattribution_horiz_black.png'))
  deck:setRect(-512, -384, 512, 384)
  ...
  -- add timer to get to title
  local timer = MOAITimer.new()
  timer:setMode(MOAITimer.NORMAL)
  timer:setSpan(2)
  timer:setListener(MOAITimer.EVENT_TIMER_END_SPAN, 
                    function () self.change_scene = true end)
  timer:start()
...
function Scene.update(self, delta_time)
  ...
  if self.change_scene then
    scene_manager:replace_scene('title_scene')
  end
end
